...
import { ApprovalMode, Storage, createDebugLogger } from "@terminuz/tui-shim";
...
function isExactRunnableSlashCommand(rawInput: string, commands: readonly SlashCommand[]): boolean {
  ...
  const handleInput = useCallback(
    (key: Key): boolean => {
      ...
      const acceptActiveCompletionSuggestion = (): string | null => {
        ...
        const targetIndex =
          completion.activeSuggestionIndex === -1 ? 0 : completion.activeSuggestionIndex;
      ...
      if (isOnCursorLine && cursorVisualColAbsolute === cpLen(lineText)) {
        ...
        if (ghostText && showCursorOpt && ghostText.text.length > 0) {
          if (ghostText.showCursorBeforeText) {
            renderedLine.push(<Text key="ghost-cursor">{chalk.inverse(" ")}</Text>);
            renderedLine.push(
            ...
            renderedLine.push(<Text key="ghost-cursor">{chalk.inverse(firstChar)}</Text>);
            if (rest.length > 0) {
              renderedLine.push(
                <Text key="ghost-rest" color={theme.text.secondary}>
                  {rest}
