# Terminuz — Contexto Completo do Agente

> Gerado automaticamente em 2026-05-12. Este documento descreve a arquitetura, ferramentas, prompts e fluxo de execução do agente Terminuz.

---

## 1. Visão Geral

**Terminuz** é um agente de IA para desenvolvimento de software local, operado pelo terminal. Ele combina um runtime de agente multi-provider, um modelo de execução com controle de permissões e uma TUI baseada em Ink (React para terminal).

```
apps/terminuz/        → entrypoint e pacote CLI publicável
...
| Variável             | Descrição                                |
| -------------------- | ---------------------------------------- |
| `TERMINUZ_PROVIDER`  | Provider padrão (ex: `openrouter`)       |
| `TERMINUZ_MODEL`     | Modelo padrão                            |
...
### Valores padrão da config (`TerminuzConfigSchema`)
...
- **System prompt**: _"You are Terminuz, a local terminal coding agent, running in BUILD mode..."_
...
- **System prompt**: _"You are Terminuz, a local terminal coding agent, running in PLAN mode..."_
...
```
You are Terminuz, a local terminal coding agent, running in BUILD mode.
...
```
You are Terminuz, a local terminal coding agent, running in PLAN mode.
...
```
You are Terminuz, a local terminal coding agent, handling a conversational turn.
...
```
You are Terminuz, a local terminal coding agent, handling a direct utility request in the terminal.
...
**Motivação:** resultados brutos de ferramentas (conteúdo de arquivos, saídas de comandos) podem somar dezenas de milhares de caracteres e causar erros de contexto no provider. O sub-agente herda _decisões e raciocínio_ do pai — se precisar de dados específicos, usa suas próprias ferramentas para buscá-los.

---

## 11. Cache

- Cache de resultados de ferramentas em disco (`.terminuz/cache/`)
...
O `AuditLogger` (`packages/core/src/security/audit-logger.ts`) registra todas as operações de permissão em `.terminuz/audit.log` com:
