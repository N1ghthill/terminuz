...
Terminuz segue uma estratégia de testes abrangente, com foco em testes unitários, integração e end-to-end. A arquitetura baseada em Effect facilita a criação de testes determinísticos e paralelizáveis.
...
describe("SessionService", () => {
  ...
  it("should handle errors gracefully", async () => {
    ...
    const exit = await Effect.runPromiseExit(program.pipe(Effect.provide(SessionServiceLive)));
  ...
  it("should retry on transient errors", async () => {
    ...
    const result = await Effect.runPromise(flakyEffect.pipe(Effect.retry({ times: 3 })));
...
describe("read_file tool", () => {
  ...
  beforeEach(async () => {
    tempDir = await mkdtemp(join(tmpdir(), "terminuz-test-"));
  });
  ...
  it("should read file content", async () => {
    ...
    const result = await Effect.runPromise(readFileTool.execute({ path: filePath }, mockContext));
